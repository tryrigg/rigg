...
export async function createCursorRuntimeSession(options: CursorRuntimeOptions): Promise<CursorRuntimeSession> {
  ...
  function clearPromptSettlement(execution: SessionExecution): void {
    if (execution.postPromptQuietPeriodTimer === undefined) {
      return
    }
    clearTimeout(execution.postPromptQuietPeriodTimer)
    execution.postPromptQuietPeriodTimer = undefined
  }

  async function settlePromptTurn(execution: SessionExecution, generation: number): Promise<void> {
    const status = execution.promptStopReason
    if (status === undefined || execution.settled || execution.settlementGeneration !== generation) {
      return
    }

    await rpc.whenIdle()
    if (execution.promptStopReason !== status || execution.settled || execution.settlementGeneration !== generation) {
      return
    }
  ...
  function schedulePromptTurnSettlement(execution: SessionExecution): void {
    if (execution.promptStopReason === undefined || execution.settled) {
      return
    }
  ...
  async function run(options_: {
    ...
    if (stopReason !== null) {
      execution.promptStopReason = stopReason
