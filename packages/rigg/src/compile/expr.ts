...
export function isWholeExpressionTemplate(template: string): boolean {
  const trimmed = template.trim()
  const matches = [...trimmed.matchAll(TEMPLATE_PATTERN)]
  return matches.length === 1 && matches[0]?.[0] === trimmed
}

export function unwrapWholeExpressionTemplate(template: string): string | undefined {
  const expressions = extractTemplateExpressions(template.trim())
  return isWholeExpressionTemplate(template) ? expressions[0] : undefined
}

export function compileExpression(source: string, expected: "bool" | "scalar" | null = null): CompiledExpression {
...
export function evaluateCompiledExpression(expression: CompiledExpression, context: EvalContext): unknown {
...
export function evaluateExpression(expression: string, context: EvalContext): unknown {
  return evaluateCompiledExpression(compileExpression(expression), context)
}

export function renderTemplate(template: string, context: EvalContext): unknown {
  const wholeExpression = unwrapWholeExpressionTemplate(template)
  if (wholeExpression !== undefined) {
    return evaluateCompiledExpression(compileExpression(wholeExpression), context)
  }

  const rendered = parseTemplateSegments(template).map((segment) =>
    segment.kind === "text"
      ? segment.value
      : String(evaluateCompiledExpression(compileExpression(segment.expression, "scalar"), context)),
  )

  return rendered.join("")
}

export function renderTemplateString(template: string, context: EvalContext): string {
...
export function resultShapeFromJsonValue(value: JsonValue): ResultShape {
  ...
  if (Array.isArray(value)) {
    let itemShape: ResultShape | undefined
    for (const item of value) {
      const candidate = resultShapeFromJsonValue(item)
      itemShape = itemShape === undefined ? candidate : mergeResultShapes(itemShape, candidate)
    }
    return itemShape === undefined ? { kind: "array" } : { kind: "array", items: itemShape }
  }

  const fields: Record<string, ResultShape> = {}
  for (const [key, item] of Object.entries(value)) {
    fields[key] = resultShapeFromJsonValue(item)
  }
...
export function mergeResultShapes(left: ResultShape, right: ResultShape): ResultShape {
  ...
  if (left.kind === "array" && right.kind === "array") {
    if (left.items === undefined) {
      return right.items === undefined ? left : right
    }
    if (right.items === undefined) {
      return left
    }
    return { kind: "array", items: mergeResultShapes(left.items, right.items) }
  }
  if (left.kind === "object" && right.kind === "object") {
    ...
    for (const key of leftKeys) {
      fields[key] = mergeResultShapes(left.fields[key] ?? AnyJsonShape, right.fields[key] ?? AnyJsonShape)
    }
    return { kind: "object", fields }
  }
...
function inferNodeShape(node: ExprNode, resolvePath: (reference: PathReference) => ResultShape): ResultShape {
  switch (node.kind) {
    case "literal":
      return resultShapeFromJsonValue(node.value)
    case "path":
  ...
  if (node.name === "fromJSON") {
    ...
    if (first?.kind === "literal" && typeof first.value === "string") {
      try {
        ...
        return jsonValue === undefined ? AnyJsonShape : resultShapeFromJsonValue(jsonValue)
      } catch {
        return AnyJsonShape
      }
    }
