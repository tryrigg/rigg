...
export function coerceInvocationInputValue(schema: InputDefinition, rawValue: unknown): unknown {
...
export function parseInvocationInputEntries(values: string[]): ParseInvocationInputEntriesResult {
...
export function findOmittedInvocationInputs(
...
export function mergePromptedInvocationInputs(
  invocationInputs: Record<string, unknown>,
...
export function normalizeInvocationInputs(
  ...
  for (const [key, schema] of Object.entries(workflow.inputs ?? {})) {
    ...
    const normalizedValue = coerceInvocationInputValue(schema, inputs[key])
    normalizedInputs[key] = normalizedValue
    errors.push(...validateInputValue(schema, normalizedValue, `inputs.${key}`))
  }
