---
title: Steps
description: Action step types — shell, codex, cursor, and write_file.
sidebar:
  order: 2
---

## shell

Runs a shell command.

```yaml
- id: test
  type: shell
  with:
    command: npm test
    result: text # none | text | json (default: text)
```

`result` controls stdout capture: `none` discards it, `text` captures as string, `json` parses as structured data.

## codex

Three actions are supported: `run`, `plan`, and `review`.

### run

Generates or modifies code.

```yaml
- id: implement
  type: codex
  with:
    action: run
    prompt: Add input validation.
    model: gpt-5.4
```

`steps.<id>.result` is always the final plain-text response.

### plan

Uses Codex's built-in Plan collaboration mode to gather context, ask clarifying questions when needed, and return a plan without mutating repo-tracked files.

```yaml
- id: draft_plan
  type: codex
  with:
    action: plan
    prompt: Clarify the scope and produce an implementation plan.
    model: gpt-5.4
```

`steps.<id>.result` is the final plain-text planning response.

### review

Scans diffs and returns review findings.

```yaml
- id: review
  type: codex
  with:
    action: review
    model: gpt-5.4
    review:
      target:
        type: uncommitted # uncommitted | base | commit
```

Target variants:

- `type: uncommitted`
- `type: base` with `branch`
- `type: commit` with `sha`

Review always returns the built-in structured result below:

```yaml
result:
  findings:
    - title: string
      body: string
      confidence_score: number
      priority: integer
      code_location:
        absolute_file_path: string
        line_range:
          start: integer
          end: integer
  overall_correctness: string
  overall_explanation: string
  overall_confidence_score: number
```

## cursor

Runs Cursor Agent through ACP and returns the final plain-text assistant response.

```yaml
- id: triage
  type: cursor
  with:
    action: run # run | ask | plan
    model: composer-2
    prompt: Summarize the diff.
```

`cursor` is ACP text-only. If you need structured JSON from Cursor, run Cursor headless through a `shell` step and parse that shell output there.

## write_file

Writes content to a file.

```yaml
- id: save
  type: write_file
  with:
    path: ${{ inputs.output_path }}
    content: ${{ steps.draft.result }}
```

## Result access

- `codex run`: `steps.draft.result`
- `codex plan`: `steps.draft_plan.result`
- `codex review`: `steps.review.result.findings`
- `cursor`: `steps.triage.result`
