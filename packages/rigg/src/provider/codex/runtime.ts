...
import { approvalResponse, assertApprovalDecision, assertResolutionKind, permApprovalResponse } from "./interaction"
...
import { parseReviewText } from "./review"
...
type TurnState = { kind: "interrupting" } | { kind: "running" } | { kind: "settled" }
...
export async function createCodexRuntimeSession(options: CodexRuntimeOptions): Promise<CodexRuntimeSession> {
  ...
  function isSettled(execution: TurnExecution): boolean {
    return execution.state.kind === "settled"
  }
  ...
  async function interruptExecution(execution: TurnExecution): Promise<void> {
    if (isSettled(execution) || execution.state.kind === "interrupting") {
      return
    }

    execution.state = { kind: "interrupting" }
