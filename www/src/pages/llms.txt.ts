...
export const GET: APIRoute = async () => {
  ...
  const docLinks = sorted
    .map((doc) => `- [${doc.data.title}](${SITE}/${doc.id}/): ${doc.data.description}`)
    .join('\n');
...
Supported agents: Codex (OpenAI), Claude Code (Anthropic). Coming soon: opencode, Kimi Code.
